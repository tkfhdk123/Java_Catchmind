   ...
   public class clientManager extends Thread{
      ...
      public void run() {
         ...
         try {
            ...
            clientOutput.remove(nickname); clientScore.remove(nickname);
            Close();
            serverMsg(nickname+"님 입장!\n 현재 플레이어 수 : "+clientOutput.size()+"/4");
      ...
      public void clientSet() {
         ...
         for(int i=0;i<clientScore.size(); i++)
            serverMsg("_CList"+nicknames[i]+":"+scores[i]+"#"+i+1);
      }
...
//                if()
//                serverMsg("//Auth " + clientOutput.get(it.); // 명령어 : 문제 출제자 랜덤 선택
//                
                //Quiz quiz = new Quiz(); quiz.start(); // 문제 출제
                //Timer tm = new Timer(); tm.start(); // 타이머 시작
                gamestart = true;
                serverMsg("_Start"); // 명령어 : 게임 시작
             }
         }else if(str.equals("_Mouse")){ // 명령어 : 마우스 좌표 수신
            serverMsg(str);
         }else if(str.equals("_Color")){ // 명령어 : 컬러 설정
            serverMsg(str);
         }else if(str.equals("_Erase")){ // 명령어 : 지우기
            serverMsg(str);
         }else if(str.equals("_ErAll")){ // 명령어 : 모두 지우기
            serverMsg(str);
         }else if(str.equals("_GmEnd")){ // 명령어 : 게임 종료 (시간 초과나 이탈자 발생으로 게임이 종료되는 경우)
            serverMsg("게임이 종료되었습니다 !");
            serverMsg(str);
            //readyPlayer = 0;
            gamestart = false;
         }
